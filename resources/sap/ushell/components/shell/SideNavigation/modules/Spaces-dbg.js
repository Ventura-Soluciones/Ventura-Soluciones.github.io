...
sap.ui.define([
    ...
    class Spaces {
        ...
        #hasSpaceIdAndPageId (oSideNavigationEntry, sSpaceId, sPageId, sAppIdHint) {
            ...
            const bSpaceIdFound = aParameters.some(function (oParameter) {
                return oParameter.name === "spaceId" && oParameter.value === sSpaceId;
            });
            const bPageIdFound = aParameters.some(function (oParameter) {
                return oParameter.name === "pageId" && oParameter.value === sPageId;
            });
            const bAppIdHintFound = aParameters.some(function (oParameter) {
                return oParameter.name === "sap-ui-app-id-hint" && oParameter.value === sAppIdHint;
            });
            const bNoPageAndNoApp = !sPageId && !sAppIdHint;

            return bSpaceIdFound && (bPageIdFound || bAppIdHintFound || bNoPageAndNoApp);
        }
