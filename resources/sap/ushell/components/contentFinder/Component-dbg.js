...
/**
 * @typedef sap.ui.model.json.JSONModel
...
sap.ui.define([
    ...
    "sap/f/library"
], function (Log, deepExtend, ObjectPath, /** @type {typeof import("sap/ui/core/UIComponent").default>} */ UIComponent, GraphQLModel, JSONModel, readUtils, utilsCdm, fLibrary) {
    "use strict";

    const LayoutType = fLibrary.LayoutType;

    const iLargeDataSetThreshold = 500;
    ...
    return UIComponent.extend("sap.ushell.components.contentFinder.Component", /** @lends sap.ushell.components.contentFinder.Component.prototype */{
        ...
        /**
         * Gets the selected visualization filter.
         * @returns {object} The selected visualization filter.
         * @since 1.134.0
         * @private
         */
        getSelectedVisualizationFilter: function () {
            this.oUiModel = this.getUiModel();
            const sSelectedVisualizationFilter = this.oUiModel.getProperty("/visualizations/filters/selected");
            const aAvailableVisualizationFilters = this.oUiModel.getProperty("/visualizations/filters/available");

            let oInitialAvailableFilterTypes = {};
            if (aAvailableVisualizationFilters?.length > 0) {
                oInitialAvailableFilterTypes = aAvailableVisualizationFilters.find((oElement) => oElement.key === sSelectedVisualizationFilter) || aAvailableVisualizationFilters[0];
            }

            return oInitialAvailableFilterTypes;
        },
         ...
         * @param {{type: sap.ui.IllustratedMessageType, title: string, description: string}} oCustomErrorMessage override the error message
         *
         * @since 1.115.0
         * @private
         */
        setVisualizationData: function (oVisualizationData, bReplaceVisualizations = false, oCustomErrorMessage) {
            ...
            oDataModel.setProperty("/visualizations/items", aExistingVisualizations.concat(aPreparedVisualizations));

            if (Number(iTotalCount) > 0) {
                oDataModel.setProperty("/visualizations/totalCount", iTotalCount);
            }
            oUiModel.setProperty("/visualizations/loaded", true);
            oUiModel.setProperty("/customErrorMessage", oCustomErrorMessage);
        ...
        updateSidebarStatus: function (sLayoutType) {
            ...
            const bShouldTreeTableBeVisible = this._shouldTreeTableBeVisible();
            oUiModel.setProperty("/largeDataSet", bShouldTreeTableBeVisible);
        },

        /**
         * @returns {boolean} Returns true if the tree table should be visible.
         * @since 1.135.0
         * @private
         */
        _shouldTreeTableBeVisible: function () {
            const oModel = this.getDataModel();
            const oTreeBinding = oModel.bindTree("/categoryTree");
            const iCount = oTreeBinding.getCount();

            return iCount > iLargeDataSetThreshold;
        },
        ...
        /**
         * Prepares and clones visualizations from the provided visualizationData and enriches them.
         *
         * @param {object[]} aVisualizationData The visualizationData from the consumer.
         * @returns {preparedVizualization[]} The prepared clone of the visualizations.
         * @since 1.113.0
         * @private
         */
        _prepareVisualizations: function (aVisualizationData) {
            return deepExtend([], aVisualizationData).reduce((aResult, oViz) => {
                ...
                if (oViz.targetAppIntent) {
                    const oTarget = readUtils.harmonizeTarget(oViz.descriptor.value["sap.flp"]?.target);
                    sLaunchUrl = utilsCdm.toHashFromTargetIntent(oViz.targetAppIntent, oTarget);
                }
