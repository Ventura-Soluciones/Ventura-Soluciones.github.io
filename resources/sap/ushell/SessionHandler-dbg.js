...
sap.ui.define([
    ...
    let bIsLogoutProcessStarted = false;
    /**
    ...
    function SessionHandler () {
        ...
        this.init = function (oConfig) {
            ...
            if (oConfig.sessionTimeoutReminderInMinutes === undefined) {
                oConfig.sessionTimeoutReminderInMinutes = 0; // Default is to show the session timeout message box with no reminder
            }

            // Handle negative values (sessionTimeoutIntervalInMinutes may be -1 for disabling the session timeout)
            if (oConfig.sessionTimeoutIntervalInMinutes < -1) {
                oConfig.sessionTimeoutIntervalInMinutes = -1;
                Log.warning("sessionTimeoutIntervalInMinutes was invalid and has been set to -1", "SessionHandler");
            }
            if (oConfig.sessionTimeoutReminderInMinutes < 0) {
                oConfig.sessionTimeoutReminderInMinutes = 0;
                Log.warning("sessionTimeoutReminderInMinutes was negative and has been set to 0", "SessionHandler");
            }

            // The timeout reminder should be adapted to zero if no timeout is configured
            if (oConfig.sessionTimeoutIntervalInMinutes <= 0) {
                oConfig.sessionTimeoutReminderInMinutes = 0;
            } else if (oConfig.sessionTimeoutIntervalInMinutes <= oConfig.sessionTimeoutReminderInMinutes) {
                // The timeout interval must always be higher than the timeout reminder, set reminder to timeout interval -1
                oConfig.sessionTimeoutReminderInMinutes = (oConfig.sessionTimeoutIntervalInMinutes - 1) < 0 ? 0 : oConfig.sessionTimeoutIntervalInMinutes - 1;
                Log.error("sessionTimeoutReminderInMinutes needs to be smaller than sessionTimeoutIntervalInMinutes." +
                    `sessionTimeoutReminderInMinutes adapted to: ${oConfig.sessionTimeoutReminderInMinutes}`, "SessionHandler");
            }
        ...
        this.monitorUserIsInactive = function (bRescheduleTimer) {
            ...
            const iTimeLastInteraction = this.timeLastInteraction();
            const iTimePopup = this.timePopup();
            const iTimeNow = this.timeNow();
            const iTimeOver = this.timeOver();
            const iRemainingTimeUntilDialog = iTimePopup - iTimeNow;
            const iTimeSinceLastInteraction = iTimeNow - iTimeLastInteraction;
            ...
            if (iTimeNow < iTimePopup && bRescheduleTimer) {
                const oMonitorUserIsInactiveTimer = setTimeout(this.monitorUserIsInactive.bind(this), iRemainingTimeUntilDialog * 1000);
        ...
        this.notifyServer = async function () {
            const timeSinceLastActionInMilliseconds = this._getCurrentDate() - new Date(this.getTimestampFromStorage());
            const timeSinceLastActionInMinutes = timeSinceLastActionInMilliseconds / (1000 * 60);

            //Setting the timeout to be sessionTimeoutIntervalInMinutes / 2 (hence the 30 instead of 60)
            const iTimeout = this.oConfig.sessionTimeoutIntervalInMinutes * 30 * 1000;
            ...
            if (timeSinceLastActionInMinutes <= this.oConfig.sessionTimeoutIntervalInMinutes) {
                ...
                PostMessageManager.sendRequestToAllApplications("sap.ushell.sessionHandler.extendSessionEvent", {}, false);
            } else {
        ...
        this.monitorCountdown = function (bIsExternalCall) {
            ...
            const iRemainingTimeInSeconds = iTimeOver - iTimeNow;
        ...
        this.checkStopBackendRequestRemainingTime = function () {
            const sTimeSinceLastActionInMilliseconds = this._getCurrentDate() - new Date(this.getTimestampFromStorage());
            const sTimeSinceLastActionInMinutes = sTimeSinceLastActionInMilliseconds / (1000 * 60);
            const sReminderIntervalInMinutes = this.oConfig.sessionTimeoutTileStopRefreshIntervalInMinutes;
            const sRemainingMillisecondsUntilTimeout = (sReminderIntervalInMinutes - sTimeSinceLastActionInMinutes) * 60 * 1000;
        ...
        this._setTilesVisibleOnHomepage = function () {
            sap.ui.require(["sap/ushell/utils"], (oUtils) => {
        ...
        this._setTilesInvisibleOnHomepage = function () {
            ...
            return new Promise((resolve, reject) => {
                return Container.getServiceAsync("FlpLaunchPage").then((LaunchPageService) => {
                    LaunchPageService.getGroups().then((aGroups) => {
                        const oEventBus = EventBus.getInstance();
                        aGroups.forEach((oGroup) => {
                            LaunchPageService.getGroupTiles(oGroup).forEach((oGroupTile) => {
        ...
        this.getLocalStorage = function () {
            if (this.oLocalStorage === undefined) {
                const oStorage = new Storage(this.oConfig && this.oConfig.sessionType || Storage.Type.local);
        ...
        this._createSystemNotification = function () {
            const fnFireNotification = function () {
                const iSessionRemainingTimeInSeconds = this.oModel.getProperty("/SessionRemainingTimeInSeconds");
                const oNotification = new Notification(oResources.i18n.getText("sessionTimeoutMessage_title"), {
        ...
        this.continueWorkingFormatter = function (iSessionRemainingTimeInSeconds) {
            const bIsTimeInMinutes = iSessionRemainingTimeInSeconds > 60;
            let sMessage;
            const sTimeUnit = bIsTimeInMinutes ? oResources.i18n.getText("sessionTimeoutMessage_units_minutes") : oResources.i18n.getText("sessionTimeoutMessage_units_seconds");
            const iSessionRemainingTime = bIsTimeInMinutes ? Math.ceil(iSessionRemainingTimeInSeconds / 60) : iSessionRemainingTimeInSeconds;
         ...
         * @private
         */
        this.continueWorkingButtonPressHandler = async function (isTriggeredBySessionHandler) {
            Log.debug(`SessionHandler.continueWorkingButtonPressHandler :: Session was extended ${isTriggeredBySessionHandler ? "automatically" : "by user"}`, "", "SessionHandler");
        ...
        this.putTimestampInStorage = function (tTimestamp) {
            ...
            const oLocalStorage = this.getLocalStorage();
            if (oLocalStorage) {
        ...
        this.logout = function () {
            return new Promise(async (resolve) => {
                ...
                const oLoading = new LoadingDialog({ text: "" });

                // post the logout event to isolated
                PostMessageManager.sendRequestToAllApplications("sap.ushell.sessionHandler.logout", {}, true).then(async () => {
                ...
                const oTimeout = window.setTimeout(async () => {
                    bIsLogoutProcessStarted = true;
                    _logout(oLoading);
                }, 4000);
            });
