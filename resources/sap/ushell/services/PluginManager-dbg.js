...
sap.ui.define([
    ...
    function PluginManager (oContainerInterface, sParameter, oServiceProperties) {
         ...
         * @param {Promise<object>} oPostMessageInterfacePromise The application postMessage interface.
         * @since 1.38
         * @private
         */
        ...
        this.filterPluginIds = function (aPluginIds, sPluginMode, sPluginCategory) {
            // determine discard mode
            let sDiscardMode;

            if (sPluginMode.startsWith("discard-filter-")) {
                sDiscardMode = "discardFilterPluginsAccordingToSimpleExpression";
            } else if (sPluginMode.startsWith("discard")) {
                sDiscardMode = "discardAlmostAllPlugins";
            } else {
                sDiscardMode = "noDiscard";
            }
            // evaluate discard mode
            switch (sDiscardMode) {
                case "discardFilterPluginsAccordingToSimpleExpression":
                // remove prefix 'discard-' from the plugin mode
                    var sPluginIdToDiscard = sPluginMode.replace("discard-filter-", "");
                    return SimpleExpression.filterByExpression(aPluginIds, sPluginIdToDiscard);
         ...
         * Values of the form <code>discard-filter-<SimpleExpression></code> can be used to filter the plugins to load.
         *
