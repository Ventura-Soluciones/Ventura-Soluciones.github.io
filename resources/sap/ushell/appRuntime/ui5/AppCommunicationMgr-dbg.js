...
sap.ui.define([
    ...
    function AppCommunicationMgr () {
        this.init = async function (bInitIframeValidMsg) {
            ...
            const oModule = await this._getPostMessageInterface();
            oModule.init(false, this.registerCommHandlers.bind(this));
        };

        this._getPostMessageInterface = async function () {
            const [PostMessagePluginInterface] = await ushellUtils.requireAsync(["sap/ushell/appIntegration/PostMessagePluginInterface"]);
            return PostMessagePluginInterface;
        };
