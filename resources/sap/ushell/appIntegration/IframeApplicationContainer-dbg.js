...
sap.ui.define([
    ...
    IframeApplicationContainer.prototype._calculateIframeWithPost = function () {
        const oUrlParams = UriParameters.fromURL(document.URL);
        const sApplicationType = this.getApplicationType();
    ...
    IframeApplicationContainer.prototype._calculateIframeUrl = function () {
        ...
        sIframeUrl = this._injectParametersFromUrl(sIframeUrl);

        this.setProperty("iframeUrl", sIframeUrl, true);
    };

    IframeApplicationContainer.prototype._injectParametersFromUrl = function (sIframeUrl) {
    ...
    IframeApplicationContainer.prototype.getPostMessageTargetOrigin = function () {
        ...
        if (this.getIframeWithPost() === true) {
            ...
            if (sIframeUrl === undefined && (new URI()).query(true).hasOwnProperty("sap-isolation-enabled")) {
    ...
    IframeApplicationContainer.prototype.sendRequest = async function (sServiceRequest, oMessageBody, bWaitForResponse = false) {
        if (!bWaitForResponse) {
            return PostMessageManager.sendRequest(
                sServiceRequest,
                oMessageBody,
                this.getPostMessageTarget(),
                this.getPostMessageTargetOrigin(),
                false
            );
        }
        ...
        const oPromise = PostMessageManager.sendRequest(
            sServiceRequest,
            oMessageBody,
            this.getPostMessageTarget(),
            this.getPostMessageTargetOrigin(),
    ...
    IframeApplicationContainer.prototype.sendBeforeAppCloseEvent = async function () {
        const oEventData = this.getBeforeAppCloseEvent && this.getBeforeAppCloseEvent();
