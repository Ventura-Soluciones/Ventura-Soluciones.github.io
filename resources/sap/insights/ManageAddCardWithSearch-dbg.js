...
sap.ui.define([
    ...
    ManageAddCardWithSearch.prototype._createMessageStripHBox = function() {
        ...
        }).addStyleClass("sapThemeHighlight-asColor aiTextWidth");
     ...
     * @private
     */
     ManageAddCardWithSearch.prototype.onChange = function(oEvent) {
        const MAX_CHAR_LIMIT = 500;
        const sQuery = this._getTextBox().getValue();
        const bValidInputLength = sQuery.length > 0 && sQuery.length <= MAX_CHAR_LIMIT;
        if (sQuery?.trim().length) {
    ...
    ManageAddCardWithSearch.prototype._processFilterTokens = function (oFilterTokens) {
        ...
        if (oFilterTokens) {
            ...
            var appName = oFilterTokens.app && oFilterTokens.app.name;
            aTokenset.push(this.i18Bundle.getText("APP_LABEL") + ": " + appName);
            if (oFilterTokens.topWithLabel) {
                aTokenset.push(oFilterTokens.topWithLabel.topLabel + ": " + oFilterTokens.topWithLabel.value);
            } else if (oFilterTokens.top) {
                aTokenset.push(this.i18Bundle.getText("TOP_LABEL") + ": " + oFilterTokens.top);
            }
