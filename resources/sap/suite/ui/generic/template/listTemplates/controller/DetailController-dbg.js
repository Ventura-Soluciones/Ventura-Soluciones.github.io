...
sap.ui.define([
	...
	JSONModel, Filter, RowSettings, FilterOperator, SapTableLibrary, FeLogger, isEmptyObject, FilterUtil) {
		"use strict";
		...
		var tController = Controller.extend("sap.suite.ui.generic.template.listTemplates.controller.DetailController", {
			...
			_applyCriticalityInfo: function(oEvent, smartTable){
				...
				if (aBindingParts && aBindingParts.length > 0) {
					...
					if (me.isResponsive) {
						this.oState.alp_ColumnListItem && this.oState.alp_ColumnListItem.bindProperty("highlight", {
							parts: aBindingParts,
			...
			_updateExpandLevelInfo: function(groupList, oBindingParams) {
				...
				if (expandLevels >= oBinding.aMaxAggregationLevel.length) {
					...
					bLevelUpdate = oBindingParams.parameters.numberOfExpandedLevels != expandLevels || this.wasAtMaxLevel;
					this.wasAtMaxLevel = false;
				}
				if (bLevelUpdate) {
					if (expandLevels >= 0) {
						oBindingParams.parameters.numberOfExpandedLevels = expandLevels;
					...
					for (var i = 0; i < paramList.length; i++) {
						...
						for (var name in param) {
							...
							if (!bPreventDuplicateFilter) {
								...
								// For nullable fields we add both empty or null filter
								var bIsNullable = false;
								if (param[name] === "") {
									var sEntitySet = this.oState.oSmartFilterbar.getEntitySet();
									var oModel = this.oState.oController.getOwnerComponent().getModel();
									bIsNullable = FilterUtil.isNullable(oModel, sEntitySet, name);
								}
								if (bIsNullable) {
