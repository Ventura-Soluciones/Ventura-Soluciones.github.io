		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
			...
			function onActionButtonPressed(oTreeNode, sActionName, bCloseColumn) {
				...
				if (bCloseColumn) {
					...
					oTemplateContract.oApplicationProxy.performAfterSideEffectExecution(fnProcessFunction, true);
				}
			}
			...
			function fnDisplayMessagePage(mParameters, mComponentsDisplayed){
				...
				var oDisplayPromise = oTargets.display(sTarget);
			...
			function fnWillBeOpenedInFullscreen(oTargetNode){
				return getTargetLayoutForTreeNode(oTargetNode) === getFullscreenLayout(oTargetNode.fCLLevel);
			}
