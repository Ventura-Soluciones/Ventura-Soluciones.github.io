...
sap.ui.define(["sap/ui/core/Element",
...
], function(Element, XMLView, FlexibleColumnLayoutHandler, StableIdHelper, FeLogger, extend, FeError){
		"use strict";
	...
	var oCrossAppNavServicePromise = UShellContainer ? UShellContainer.getServiceAsync("Navigation") : (function(){
			var i = 0;
			var fnCreateEmptyAppState = function(){
				i++;
				return {
					getKey: function(){
						return "" + i;
			...
			return Promise.resolve({
				createEmptyAppState: function(){
					return Promise.resolve(fnCreateEmptyAppState());
				},
		...
		function createTargetForTreeNode(oNavigationControllerProxy, oTreeNode, sHostId, sHostAggregation){
			...
			var fnRegisterTreeNodeFirstDisplay = function(){
				var bIsReallyDisplayed = oNavigationControllerProxy.treeNodeFirstDisplay(oTreeNode); // The NavigationController reports back, whether we really will display this target (and it thus has done the needful)
				if (bIsReallyDisplayed){
		...
		function generateRoutingStructure(oTemplateContract) {
			...
			return oModel.getMetaModel().loaded().then(function(){
				...
				var oSapAppManifestEntry = oTemplateContract.oAppComponent.getManifestEntry("sap.app");
				var oSapUiManifestEntry = oTemplateContract.oAppComponent.getManifestEntry("sap.ui");
				var oTopTreeNode = fnAddInitialTreeNodeProperties({
					...
					level: 0,
					fCLLevel: 0,
					headerTitle: oSapAppManifestEntry.title,
					text: oSapAppManifestEntry.description,
					titleIconUrl: oSapUiManifestEntry.icons && oSapUiManifestEntry.icons.icon
				});
			...
			oTreeNode.unbindElement = function(oElement, bForSpecificModel){
				...
				if (oElement){
					...
					for (var sElementId in mBindings){
						fnUnbind(Element.getElementById(sElementId), sElementId);
					}
				}
			};
		}
		...
		function getCrossAppNavServicePromise(){
			return oCrossAppNavServicePromise;
		}

		return {
			generateRoutingStructure: generateRoutingStructure,
			getCrossAppNavServicePromise: getCrossAppNavServicePromise,
