...
sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		function getTargetIdentityPromise(oTargetIdentity) {
		...
		function fnOpenContextInNewTab(oSourceNode, oNavigationContext) {
			...
			var oTargetIdentityPromise = oTargetIdentityInfo.promise;
			return oTargetIdentityPromise.then(getTargetIdentityPromise).then(function (sTargetUrl) {
		...
		// Returns the URL for navigation context
		function fnGetTargetIdentityForContext(oSourceNode, oNavigationContext) {
			var oTargetIdentityInfo = getTargetIdentityInfoForContext({ treeNode: oSourceNode }, oNavigationContext, true, false);
			var oTargetIdentityPromise = oTargetIdentityInfo.promise;
			return oTargetIdentityPromise.then(getTargetIdentityPromise).catch(function (oError) {
				oLogger.error("Error while getting the URL for the context '" + oNavigationContext.getPath() + "': " + oError);
			});
		}
		...
		oNavigationControllerProxy.getTargetIdentityForContext = fnGetTargetIdentityForContext;
