...
sap.ui.define([
	...
	"sap/m/IllustratedMessageType"
		...
		var IconColor = SapCoreLibrary.IconColor;
		...
		*/
		 ...
		 * @param {string} sActionLabel Custom label for message header title used only for success/information message type
		 * @param {Object} oActionButtonConfig config object, if you want to pass an action and callback for dialog close button. It expects following properties:
		...
		function fnHandleTransientMessages(oTemplateContract, sActionLabel, oActionButtonConfig) {
			...
			return new Promise(function(fnResolve){
				...
				var oDialogFragmentController = {
					...
					afterClose: fnDialogClose,
					onBackButtonPress: function () {
						oSettingModel.setProperty("/backButtonVisible", false);
						oMessageView.navigateBack();
					},
					onMessageSelect: function () {
						var oFirstFocusableButton = oDialog.getButtons().find(function(oButton){
				...
				oTemplateContract.oApplicationProxy.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.MessageDialog", oDialogFragmentController, "settings", fnOnFragmentCreated, false, true).then(function(odialog){
					...
					oSettingModel.setProperty("/title", (sState !== ValueState.Error && sState !== ValueState.Warning && sActionLabel) || oMsgDialogHeaderInfo.sTitle);
		...
		function getSeverityIconFromIconPool(messageType) {
		...
		function getIconColor(messageType) {
			switch (messageType) {
				case ValueState.Error:
		...
		function deleteMessages(aMessageIdsToBeDeleted) {
			var aMessagesToBeDeleted = [];
			aMessageIdsToBeDeleted.forEach(function (sMessageId) {
				var oMessage = getMessageById(sMessageId);
				if (oMessage) {
					aMessagesToBeDeleted.push(oMessage);
				}
			});
