...
sap.ui.define(["sap/base/Log", "sap/insights/CardHelper", "sap/m/Button", "sap/m/CheckBox", "sap/m/CustomListItem", "sap/m/HBox", "sap/m/IconTabBar", "sap/m/IconTabFilter", "sap/m/Input", "sap/m/Label", "sap/m/library", "sap/m/List", "sap/m/MessageStrip", "sap/m/MessageToast", "sap/m/ObjectStatus", "sap/m/OverflowToolbar", "sap/m/Panel", "sap/m/Text", "sap/m/Title", "sap/m/Toolbar", "sap/m/ToolbarSpacer", "sap/m/VBox", "sap/ui/core/Element", "sap/ui/core/EventBus", "sap/ui/core/library", "sap/ui/layout/form/SimpleForm", "sap/ui/model/json/JSONModel", "sap/ui/unified/FileUploader", "sap/ushell/Container", "./BaseSettingsPanel", "./utils/Accessibility", "./utils/AppManager", "./utils/Constants", "./utils/FESRUtil", "./utils/HttpHelper", "./utils/PageManager", "./utils/PersonalisationUtils", "./utils/UshellPersonalizer"], function (Log, CardHelper, Button, CheckBox, CustomListItem, HBox, IconTabBar, IconTabFilter, Input, Label, sap_m_library, List, MessageStrip, MessageToast, ObjectStatus, OverflowToolbar, Panel, Text, Title, Toolbar, ToolbarSpacer, VBox, Element, EventBus, sap_ui_core_library, SimpleForm, JSONModel, FileUploader, Container, __BaseSettingsPanel, ___utils_Accessibility, __AppManager, ___utils_Constants, ___utils_FESRUtil, __HttpHelper, __PageManager, __PersonalisationUtils, __UShellPersonalizer) {
  ...
  const AdvancedSettingsPanel = BaseSettingsPanel.extend("sap.cux.home.AdvancedSettingsPanel", {
    ...
    setExportSectionList: function _setExportSectionList() {
      ...
      exportSectionsList.bindItems({
        ...
        template: new CustomListItem(`${this.getId()}--exportCustomListItem`, {
          ...
          content: [new HBox(`${this.getId()}--exportContentBox`, {
            alignItems: "Center",
            items: [new CheckBox(`${this.getId()}--exportListItemCheck`, {
              select: this.onSectionsSelectionChange.bind(this, false),
              selected: "{advsettings>selected}",
              enabled: "{advsettings>enabled}"
            }), new Text(`${this.getId()}--exportListItemText`, {
     ...
     */
    _formatAccDescription: function _formatAccDescription(title) {
    ...
    loadUserPersonalizationData: function _loadUserPersonalizationData() {
      try {
        ...
        return Promise.resolve(_this7._getPersonalizationData()).then(function (persData) {
          ...
          return Promise.resolve(Promise.all([_this7.oAppManagerInstance._getSections(true), _this7.getInsightCards()])).then(function (_ref) {
            let [favSections, insightModel] = _ref;
      ...
      try {
        ...
        return Promise.resolve(_catch(function () {
          return Promise.resolve(CardHelper.getServiceAsync()).then(function (_getServiceAsync) {
            _this10.cardHelperInstance = _getServiceAsync;
            return Promise.resolve(_this10.cardHelperInstance._getUserAllCardModel()).then(function (oUserVisibleCardModel) {
    ...
    _getRecommendationSettingsPanel: function _getRecommendationSettingsPanel() {
      try {
        const _this11 = this;
        return Promise.resolve(_this11._getPersonalizationData()).then(function (persData) {
          if (!_this11._recommendationSettingsPanel) {
            const panelId = _this11.getId() + "--recommendationSettingsPanel";
            _this11._recommendationSettingsPanel = new Panel(panelId, {
              headerText: _this11._i18nBundle.getText("recommendationSettingsHeader"),
              expanded: true,
              expandable: true,
              content: [new Text({
                id: `${panelId}-container-subHeader`,
                text: _this11._i18nBundle.getText("recommendationSettingsSubHeader")
              }), new HBox({
                id: `${panelId}-container`,
                items: [new CheckBox({
                  id: `${panelId}-container-checkBox`,
                  selected: persData.showRecommendation ?? true,
                  select: event => _this11.onRecommendationSettingChange(event),
                  ariaLabelledBy: [`${panelId}-container-subHeader`, `${panelId}-container-label`]
                }), new Text({
                  id: `${panelId}-container-label`,
                  text: _this11._i18nBundle.getText("recommendationSettingsCheckboxLabel")
                })],
                alignItems: FlexAlignItems.Center
              }).addStyleClass("sapUiSmallMarginTop")]
            }).addStyleClass("sapUiSmallMarginTop");
          }
          return _this11._recommendationSettingsPanel;
        });
      } catch (e) {
        return Promise.reject(e);
      }
    ...
    _setRecommendationSettingsPanel: function _setRecommendationSettingsPanel() {
      try {
        const _this12 = this;
        _this12.oDetailPage.setBusy(true);
        return Promise.resolve(_this12._getRecommendationSettingsPanel()).then(function (recommendationSettingsPanel) {
          if (recommendationSettingsPanel) {
            _this12.oContentVBox.addItem(recommendationSettingsPanel);
            _this12.oDetailPage.setBusy(false);
          }
        });
      } catch (e) {
        return Promise.reject(e);
    ...
    onRecommendationSettingChange: function _onRecommendationSettingChange(event) {
      try {
        ...
        _this13.oEventBus.publish("importChannel", "recommendationSettingChanged", {
          showRecommendation
        });
        return Promise.resolve(_this13._getPersonalizationData()).then(function (oPersData) {
          void _this13.oPersonalizerInstance.write({
            ...oPersData,
            showRecommendation
          });
        });
