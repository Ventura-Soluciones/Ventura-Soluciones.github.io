  ...
  const CardsPanel = BasePanel.extend("sap.cux.home.CardsPanel", {
    ...
    _addRuntimeHost: function _addRuntimeHost() {
      const _this4 = this,
        ...
        _this7 = this,
        _this8 = this;
      const host = this.getProperty("host") || new InsightsInMemoryCachingHost("runtimeHost");
      ...
      try {
        ...
        function _temp8() {
          ...
          const _temp5 = function () {
            if (_this10.aiFlagEnabled === undefined) {
              return Promise.resolve(_this10.updateAICardCreationStatus()).then(function () {});
            }
          }();
        ...
        const _temp7 = function () {
          if (!_this10.cardHelperInstance) {
            return Promise.resolve(CardHelper.getServiceAsync()).then(function (_getServiceAsync2) {
              _this10.cardHelperInstance = _getServiceAsync2;
            });
          }
        }();
            ...
            void function () {
              try {
                const _temp14 = _catch(function () {
                  ...
                  return Promise.resolve(_this11.cardHelperInstance?._getUserVisibleCardModel()).then(function (userVisibleCardModel) {
    ...
    setNewVisibleCards: function _setNewVisibleCards() {
      try {
        const _this12 = this;
        return Promise.resolve(_this12.cardHelperInstance._getUserVisibleCardModel()).then(function (visibleCardModel) {
        ...
        return Promise.resolve(function () {
          if (manifestIds.length) {
            ...
            return Promise.resolve(_this13.regenerateCards(uniqueManifestDetails, manifestIds)).then(function (aUpdatedCards) {
              let _exit = false;
              function _temp16(_result2) {
                return _exit ? _result2 : Promise.resolve(_this13.setNewVisibleCards()).then(function () {});
              }
     ...
     * @private
     */
    _createAddInsightCardMenuItem: function _createAddInsightCardMenuItem(id, fesrId) {
      const menuItem = new MenuItem(`${this.getId()}-${id}`, {
     ...
     * @private
     */
    _generateInsightCardsWithAI: function _generateInsightCardsWithAI() {
      const _this23 = this;
      this.cardHelperInstance.showAddCardWithSearchDialog(function () {
        try {
          return Promise.resolve(_this23.rerenderCards(true)).then(function () {});
        } catch (e) {
          return Promise.reject(e);
        }
      });
     ...
     * @private
     */
    updateAICardCreationStatus: function _updateAICardCreationStatus() {
      try {
        const _this24 = this;
        let aPromises = [Container.getServiceAsync("Navigation"), _this24.appManagerInstance.isFeatureEnabled(FEATURE_TOGGLES.AI_GENERATED_CARD)];
        return Promise.resolve(Promise.all(aPromises).then(aResult => {
          let oNavigationService = aResult[0];
          let enabled = aResult[1];
          // if smart toggle enabled check if navigation supported
          if (enabled) {
            oNavigationService.isNavigationSupported([{
              target: {
                semanticObject: "IntelligentPrompt",
                action: "personalize"
              }
            }]).then(aResponses => {
              _this24.aiFlagEnabled = aResponses[0].supported || false;
              if (_this24.aiFlagEnabled && _this24._isDesktopDevice()) {
                const addInsightCardMenuItem = _this24._createAddInsightCardMenuItem(cardsMenuItems.AI_INSIGHT_CARD, "AddAIInsightCard");
                _this24.addAggregation("menuItems", addInsightCardMenuItem);
                _this24._sortMenuItems(sortedMenuItems);
                const containerAddInsightMenuItem = _this24._createAddInsightCardMenuItem(cardsContainerMenuItems.AI_INSIGHT_CARD, "AddAIInsightCard");
                const CardsPanelName = _this24.getMetadata().getName();
                const InsightsContainerName = _this24._getInsightsContainer().getMetadata().getName();
                const parentContainerContent = _this24._getInsightsContainer().getContent();
                const TilesPanelName = parentContainerContent.find(panel => panel instanceof TilesPanel)?.getMetadata()?.getName() || "";

                // Check if the specific panels exist within the insights container
                const cardPanelExist = checkPanelExists(_this24.insightsContainer, InsightsContainerName, CardsPanelName);
                const tilePanelExist = checkPanelExists(_this24.insightsContainer, InsightsContainerName, TilesPanelName);
                ...
                _this24.insightsContainer?.addCommonMenuItems([containerAddInsightMenuItem]);
              }
            }).catch(error => {
              Log.warning(error instanceof Error ? error.message : String(error));
            });
