  ...
  const FavAppPanel = BaseAppPersPanel.extend("sap.cux.home.FavAppPanel", {
    ...
    _switchToRecommendedIfNoFavApps: function _switchToRecommendedIfNoFavApps(apps, groups) {
      try {
        ...
        const _temp5 = function () {
          if (_this7._isInitialLoad && [...apps, ...groups].length === 0) {
            container.setProperty?.("selectedKey", "recommendedApps");
            return Promise.resolve(container.load()).then(function () {});
          }
        }();
    ...
    _getAppActions: function _getAppActions(group, index, appVisualization) {
      ...
      const selectColorMenuItem = new MenuItem({
        id: groupId && !defaultSection ? `${groupId}--selectColorGroupAppMenuItem--${index}` : `${this.getKey()}--selectColorAppMenuItem--${index}`,
      ...
      if (this._getAllowedMoveGroups(groupId)?.length >= 1) {
        const moveToMenuItem = new MenuItem({
          id: groupId && !defaultSection ? `${groupId}--moveToGroupAppMenuItem--${index}` : `${this.getKey()}--moveToMenuItem--${index}`,
      ...
      const removeFromMyHomeMenuItem = new MenuItem({
        id: groupId && !defaultSection ? `${groupId}--removeFromMyHomeGroupAppMenuItem--${index}` : `${this.getKey()}--removeFromMyHomeMenuItem--${index}`,
    ...
    _openMoveToGroupPopover: function _openMoveToGroupPopover(event, sourceGroup) {
      ...
      list.removeAllItems();
      this._getAllowedMoveGroups(sourceGroupId).forEach(targetGroup => list.addItem(new StandardListItem(recycleId(`${this.getId()}-moveTo-${app.getId()}`), {
