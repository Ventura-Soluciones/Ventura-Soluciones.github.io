 ...
 */
sap.ui.define(["sap/m/Button", "sap/m/FlexBox", "sap/m/IconTabBar", "sap/m/IconTabFilter", "sap/m/IllustratedMessage", "sap/m/IllustratedMessageSize", "sap/m/IllustratedMessageType", "sap/m/library", "./AppsAdditionPanel", "./BaseSettingsDialog", "./InsightsAdditionPanel"], function (Button, FlexBox, IconTabBar, IconTabFilter, IllustratedMessage, IllustratedMessageSize, IllustratedMessageType, sap_m_library, __AppsAdditionPanel, __BaseSettingsDialog, __InsightsAdditionPanel) {
  "use strict";
  ...
  const ContentAdditionDialog = BaseSettingsDialog.extend("sap.cux.home.ContentAdditionDialog", {
          ...
          if (!_this.panelsAdded) {
            ...
            const _temp = _forOf(panels, function (panel) {
              return Promise.resolve(panel.isSupported()).then(function (isSupported) {
                ...
                const iconTabFilter = new IconTabFilter(`${_this.getId()}-${key}`, {
                  key,
     ...
     */
    _getPanel: function _getPanel(key) {
      return this.getPanels().find(panel => panel.getProperty("key") === key);
    },
    /**
    ...
    updateActionButtons: function _updateActionButtons() {
      ...
      const selectedKey = this.iconTabBar.getSelectedKey() || this.getPanels()[0]?.getProperty("key");
      this.removeAllButtons();

      // add action buttons from the selected panel
      this._getPanel(selectedKey).getActionButtons().forEach(button => {
...
//# sourceMappingURL=ContentAdditionDialog-dbg-dbg.js.map
