...
sap.ui.define([
	...
	Viewport.prototype.setRenderMode = function(renderMode) {
		...
		this.setShouldRenderFrame(false);
		return this;
	};
	...
	Viewport.prototype.render = function() {
		...
		try {
			...
			const mustUpdateDynamicObjectsAgain = someObjectsWereUpdated && (nativeCamera.near <= 0 || nativeCamera.far <= 0);
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				...
				if (!boundingBox.isEmpty()) {
					...
					if (mustUpdateDynamicObjectsAgain) {
						// Note: dynamic objects billboarding needs camera Z near/far (see Billboard.prototype._update)...
						// But znear/zfar depend on scene bbox which includes dynamic objects bboxes
						// Usually it is fine to use Z near/far computed in previous frame, but it would not work
						// if Z near and far equal to -1.0 default values for "auto evaluate", so we must
						// update dynamic objects twice in such case (it happens on view activation only).
