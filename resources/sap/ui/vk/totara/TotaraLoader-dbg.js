...
sap.ui.define([
	...
	TotaraLoader.prototype.getSessionId = function() {
		return this._sessionId;
	};

	TotaraLoader.prototype.setSessionId = function(sessionId) {
	...
	TotaraLoader.prototype.getConsumptionScenario = function() {
		return this._consumptionScenario;
	};

	TotaraLoader.prototype.setConsumptionScenario = function(consumptionScenario) {
		this._consumptionScenario = consumptionScenario;
		this.postMessage({
			method: Command.setConsumptionScenario,
		...
		(authorizationHandler && authorizationHandler(this.getUrl()) || Promise.resolve())
			.then(function(accessTokenResponse) {
				...
				that.postMessage({
					...
					consumptionScenario: that.getConsumptionScenario()
				});
	...
	TotaraLoader.prototype.requestViewGroup = function(sceneVeId, viewGroupId, includeAnimation, forceRefresh) {
		if (!viewGroupId) {
	...
	TotaraLoader.prototype.processContextCommand = function(context, name, jsonContent, binaryContent, isInitial) {
		...
		try {
			...
			Log.error("An error happened while processing an HTTP response", err);
			result = {
				error: err
			};
		}
	...
	TotaraLoader.prototype.processCommand = function(name, jsonContent, binaryContent, isInitial) {
		...
		switch (name) {
			case Command.setScene:
				...
				break;
			case Command.setTree: // fall through
			case Command.setTreeNode: // fall through
			case Command.notifyFinishedTree: // fall through

			case Command.setView: // fall through
			case Command.setViewNode: // fall through
			case Command.setViewGroup: // fall through
			case Command.notifyFinishedView: // fall through

			case Command.setCamera: // fall through
			case Command.allocateMeshBuffer: // fall through
			case Command.setMesh: // fall through
			case Command.setMaterial: // fall through
			case Command.setGeometry: // fall through
			case Command.setImage: // fall through
			case Command.setSvgImage: // fall through
			case Command.setAnnotation: // fall through
			case Command.setLineStyle: // fall through
			case Command.setFillStyle: // fall through
			case Command.setTextStyle: // fall through
			case Command.setParametric: // fall through
			case Command.setPointCloudGroup: // fall through

			case Command.setPlayback: // fall through
			case Command.setHighlightStyle: // fall through
			case Command.setSequence: // fall through
			case Command.setTrack: // fall through

			case Command.suppressSendRequests: // fall through
			case Command.unsuppressSendRequests:
			case Command.setMarker:
	...
	function getBatchSizeInfo(streamingUrl, cachingUrl, sceneId, sessionId, maxUrlLength) {
		...
		batchedCommands.forEach(function(batchedCommand) {
			batchSizeInfo[batchedCommand] = {
				maxBatchStringLength: getMaxBatchStringLength(batchedCommand, streamingUrl, cachingUrl, sceneId, sessionId, maxUrlLength),
