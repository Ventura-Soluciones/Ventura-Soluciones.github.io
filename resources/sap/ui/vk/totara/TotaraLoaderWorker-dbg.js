...
(function() {
	...
	// Session ID can be used to trace calls from web browser apps through backend services within one session.
	var sessionId;
	...
	// The value to send in X-ConsumptionScenario HTTP request headers.
	var consumptionScenarioHeaderValue;
	...
	function executeRequest(url, responseType) {
		return new Promise(function(resolve, reject) {
			...
			if (consumptionScenarioHeaderValue) {
				xhr.setRequestHeader("X-ConsumptionScenario", consumptionScenarioHeaderValue.toUpperCase());
			}
			xhr.setRequestHeader("X-CorrelationID", uuidv4());
	...
	function sendGetView(data) {
		...
		sendRequest(request, data, "text")
			.then(function(response) {
				...
				const pointCloudGroups = json.pointCloudGroups;
				...
				if (nodes) {
					...
					nodes.forEach(function(node) {
						...
						if ("marker" in node && markers) {
							markers[node.marker].makrerNodeSid = node.sid;
						}

						if ("pointCloudGroup" in node && pointCloudGroups) {
							const pointCloudGroup = pointCloudGroups[node.pointCloudGroup];
							node.pointCloudGroupId = pointCloudGroup.id;
							pointCloudGroup.nodeId = node.sid;
						}

						if ("image" in node && images) {
							const imageInfo = images[node.image];
							node.imageId = imageInfo.id;
							node.width = imageInfo.width;
							node.height = imageInfo.height;
						}
					});
				}
				...
				if (pointCloudGroups) {
					commands.push({
						name: "setPointCloudGroup",
						jsonContent: {
							sceneId: sceneId,
							pointCloudGroups: json.pointCloudGroups
						},
						isInitial: isInitial
					});
				}
	...
	function sendGetImage(data) {
		var request;
		const usedBySvgImage = data.usedBy == "SvgImage";
		if (data.materialId) {
		...
		} else if (usedBySvgImage) {
			request = "scenes/" + data.sceneId + "/images/" + data.imageId;
			sendRequest(request, data, "text")
				.then(function(response) {
					processCommands([{
						name: "setSvgImage",
	...
	self.onmessage = function(event) {
		...
		switch (data.method) {
			...
			case "initializeConnection": {
				...
				consumptionScenarioHeaderValue = data.consumptionScenario;
				break;
			}
			case "close": {
				self.close();
