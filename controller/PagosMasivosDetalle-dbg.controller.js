sap.ui.define(["sap/ui/core/mvc/Controller", "sap/m/MessageToast", "sap/ui/model/json/JSONModel", "sap/m/MessageBox", "../model/formatter", "./modules/UserManager", "./modules/MenuManager", "./modules/PagosMasivosManager", "./modules/NavigationManager", "sap/ui/model/Filter", "sap/ui/model/FilterOperator"], function (Controller, MessageToast, JSONModel, MessageBox, __formatter, ___modules_UserManager, ___modules_MenuManager, ___modules_PagosMasivosManager, ___modules_NavigationManager, Filter, FilterOperator) {
  "use strict";
   ...
   */
  const PagosMasivosDetalle = Controller.extend("com.vs.extension.finanb1.controller.PagosMasivosDetalle", {
    ...
    _debouncedSearch: function _debouncedSearch(iDelay) {
    ...
    onSearch: function _onSearch() {
      const aTableFilters = this.oFilterBar.getFilterGroupItems().reduce((aResult, oFilterGroupItem) => {
        ...
        if (oControl && oControl.getSelectedKeys) {
          ...
          const aFilters = aSelectedKeys.map(sSelectedKey => {
            ...
            if (fieldPath === "Name") fieldPath = "DocType";else if (fieldPath === "Category") fieldPath = "CardCode";else if (fieldPath === "SupplierName") fieldPath = "BankCode";
            return new Filter({
              path: fieldPath,
          ...
          if (sValue && sValue !== "") {
            ...
            if (aDateRange.length === 2) {
              ...
              if (sStartDate && sEndDate && sStartDate !== "" && sEndDate !== "") {
                aResult.push(new Filter({
                  path: "DocDate",
                  // Mapear al campo de fecha del documento
    ...
    _clearTableFilters: function _clearTableFilters() {
    ...
    _logTableData: function _logTableData() {
    ...
    onNuevaPlanilla: function _onNuevaPlanilla() {
    ...
    mockCancelPlanillaAPI: function _mockCancelPlanillaAPI(planillaId) {
    ...
    onCuentaBancariaChange: function _onCuentaBancariaChange(oEvent) {
      ...
      const cuentasBancariasModel = oView.getModel("cuentasBancarias");
      ...
      const selectedAccount = cuentasBancarias.value.find(account => account.AccNo === selectedKey);
      if (!selectedAccount) {
    ...
    onEjecutarPlanilla: function _onEjecutarPlanilla() {
    ...
    mockCreatePlanillaAPI: function _mockCreatePlanillaAPI(planillaData) {
      ...
      setTimeout(() => {
        ...
        const mockResponse = {
          ...
          planillaId: Math.floor(Math.random() * 1000) + 100,
          // ID aleatorio
          timestamp: new Date().toISOString(),
          createdPlanilla: {
            ...planillaData,
    ...
    mockExecutePlanillaAPI: function _mockExecutePlanillaAPI(planillaData) {
    ...
    validateNewPlanilla: function _validateNewPlanilla(planillaData) {
    ...
    onDocumentDialogConfirm: function _onDocumentDialogConfirm() {
      ...
      const oPlanillaModel = oView.getModel("planilla");
      ...
      const existingDocNums = currentDocuments.map(doc => doc.U_NUMDOC);
      ...
      aSelectedItems.forEach(oItem => {
        ...
        const newLine = {
          ...
          U_PAGEXT: 0,
          U_RETLOC: 0,
          // Valor por defecto 0 ya que RetLoc no existe en documentos disponibles
          U_RETEXT: 0,
          ...
          U_PAGLOC0: documentData.Balance,
          // Usar Balance como PAGLOC0 ya que PAGLOC0 no existe
          U_PAGEXT0: 0,
          U_RETLOC0: 0,
          // Valor por defecto 0 ya que RetLoc no existe en documentos disponibles
          U_RETEXT0: 0,
      ...
      // Cerrar el diálogo
      const oDialog = oView.byId("documentSelectionDialog");
      if (oDialog) {
        oDialog.close();
      }
    ...
    updateAlreadyAddedStatus: function _updateAlreadyAddedStatus() {
      this.markAlreadyAddedDocuments();
    },
    onDeletePlanillaLine: function _onDeletePlanillaLine(oEvent) {
      ...
      MessageBox.confirm(`¿Está seguro que desea eliminar el documento ${lineData.U_NUMDOC}?`, {
    ...
    deletePlanillaLine: function _deletePlanillaLine(lineIndex) {
      ...
      const updatedLines = planillaData.VS_PMP1Collection.filter((line, index) => index !== lineIndex);
      ...
      MessageToast.show("Línea eliminada correctamente", {
        duration: 3000
      });
    ...
    recalculatePlanillaTotals: function _recalculatePlanillaTotals(planillaData) {
      ...
      // Calcular totales
      const totalImporte = lines.reduce((sum, line) => sum + (line.U_IMPORTE || 0), 0);
      const totalRetencion = lines.reduce((sum, line) => sum + (line.U_RETLOC || 0), 0);
      const totalPagoFinal = lines.reduce((sum, line) => sum + (line.U_PAGLOC0 || 0), 0);
    ...
    onDeleteSelectedLines: function _onDeleteSelectedLines() {
      ...
      // Get the table
      const oTable = oView.byId("planillaLinesTable");
      if (!oTable) {
      ...
      const documentsToDelete = selectedItems.map(item => {
      ...
      const documentNames = documentsToDelete.map(doc => doc.U_NUMDOC).join(", ");
      MessageBox.confirm(`¿Está seguro que desea eliminar los siguientes documentos?\n${documentNames}`, {
    ...
    deleteSelectedPlanillaLines: function _deleteSelectedPlanillaLines(documentsToDelete) {
      ...
      const documentNumbersToDelete = documentsToDelete.map(doc => doc.U_NUMDOC);

      // Eliminar las líneas de la colección
      const updatedLines = planillaData.VS_PMP1Collection.filter(line => !documentNumbersToDelete.includes(line.U_NUMDOC));
      ...
      MessageToast.show(`${documentsToDelete.length} línea(s) eliminada(s) correctamente`, {
        duration: 3000
      });
    ...
    onDocumentDialogAfterOpen: function _onDocumentDialogAfterOpen() {
      ...
      const oPlanillaModel = oView.getModel("planilla");
      const oModel = oView.getModel("oModel");
    ...
    markAlreadyAddedDocuments: function _markAlreadyAddedDocuments() {
      ...
      const existingDocNums = (planillaData.VS_PMP1Collection || []).map(doc => doc.U_NUMDOC);
      ...
      availableDocuments.documents.forEach((doc, index) => {
        doc.alreadyInPlanilla = false;
  ...
  return PagosMasivosDetalle;
});
//# sourceMappingURL=PagosMasivosDetalle-dbg.controller.js.map
